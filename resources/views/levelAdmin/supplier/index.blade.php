            ...
            <div class="card border-0 shadow-sm rounded">
                <div class="card-body">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            @forelse ($supplier as $index => $dataSupplier)
                                <tr>
                                    ...
                                    <td>{{ $dataSupplier->id_barang }}</td>
                                    <td class="text-center">
                                        <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('admin.Supplier.destroy', $dataSupplier->id_supplier) }}" method="POST">
                                            <a href="{{ route('admin.Supplier.show', $dataSupplier->id_supplier) }}" class="btn btn-sm btn-success">SHOW</a>
                                            <a href="{{ route('admin.Supplier.edit', $dataSupplier->id_supplier) }}" class="btn btn-sm btn-primary">EDIT</a>
                                            @csrf
                                            @method('DELETE')
